...
)
{
    ...
    *sock_fd = socket(AF_NETLINK, SOCK_RAW, NETLINK_ROUTE);
    ...
    if (*sock_fd < 0) {
        ...
        /* LOG(ERROR) */
        m46e_logging(LOG_ERR, "Cannot open netlink socket. errno=%d\n", errno);
    ...
    if (sysret < 0) {
        ...
        /* LOG(ERROR) */
        m46e_logging(LOG_ERR, "Cannot bind netlink socket. errno=%d\n", errno);
    ...
    if (getsockname(*sock_fd, (struct sockaddr*)local, &addr_len) < 0) {
        ...
        /* LOG(ERROR) */
        m46e_logging(LOG_ERR, "Cannot bind netlink getsockname. errno=%d\n", errno);
    ...
    if ((addr_len != sizeof(*local)) || (local->nl_family != AF_NETLINK)) {
        ...
        /* LOG(ERROR) */
        m46e_logging(LOG_ERR, "Getsockname wrong socket address. length=%d, nl_family=%d\n", 
...
)
{
    ...
    // 要求送信
    ret = m46e_netlink_send(sock_fd, seq, nlm, errcd);
    if(ret != RESULT_OK){
        return ret;
    }

    // 応答受信
    ret = m46e_netlink_recv(sock_fd, local_sa, seq, errcd, m46e_netlink_parse_ack, NULL);
    if (ret != RESULT_OK){
        return ret;
    }
...
)
{
    ...
    if (NLMSG_ALIGN(n->nlmsg_len) + len > maxlen) {
        ...
        m46e_logging(LOG_ERR, " LEN ERROR\n");
        return RESULT_NG;
    }
    rta = m46e_nlmsg_tail(n);
    rta->rta_type = type;
    rta->rta_len = len;
...
{
    struct rtattr *rtattr = m46e_nlmsg_tail(n);

    m46e_netlink_addattr_l(n, maxlen, attr, NULL, 0);
...
{
    attr->rta_len = (void *)m46e_nlmsg_tail(n) - (void *)attr;
}
