    ...
    if(ret > 0){
           ...
           cmsg->cmsg_type  == SCM_CREDENTIALS)
        {
            ...
            if(cred->uid && (cred->uid != getuid() || cred->gid != getgid())) {
                 m46e_logging(LOG_INFO, "Invalid user or group. message denied. (user=%d, group=%d)", cred->uid, cred->gid);
                 ret = -EACCES;
            }
        }
    }
